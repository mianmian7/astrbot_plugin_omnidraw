...
class PluginConfig:
    ...
    auto_search_trigger_mode: str  # "keyword" | "always" | "manual"
    ...
    def from_dict(cls, config_dict: Dict[str, Any], data_dir: str) -> "PluginConfig":
        ...
        # 自动搜索参考图配置（从顶层读取）
        enable_auto_search_refs = _to_bool(config_dict.get("enable_auto_search_refs", True))
        auto_search_trigger_mode = str(config_dict.get("auto_search_trigger_mode", "keyword")).strip() or "keyword"
        auto_search_keywords = _split_csv_or_lines(config_dict.get("auto_search_keywords", "嘉然,向晚,乃琳,贝拉,A-SOUL"))
        auto_search_sites = _split_csv_or_lines(config_dict.get("auto_search_sites", "moegirl.org.cn,baidu.com,bilibili.com"))
        auto_search_max_refs = _to_int(config_dict.get("auto_search_max_refs", 2), 2, minimum=1)
        auto_search_api_url = str(config_dict.get("auto_search_api_url", "")).strip()
        auto_search_api_key = str(config_dict.get("auto_search_api_key", "")).strip()
        ...
        return cls(
            ...
            auto_search_trigger_mode=auto_search_trigger_mode,
